...
const FEATURE_GATE_CATEGORIES: Record<string, { icon: string; color: string; gates: { name: string; description: string; state: string }[] }> = {
  Storage: {
    icon: 'mdi:harddisk',
    color: '#ff9800',
    gates: [
      ...
      { name: 'HotplugVolumes', description: 'Hot-plug storage disks to running VMs', state: 'Alpha' },
  ...
  Network: {
    ...
    gates: [
      ...
      { name: 'HotplugNICs', description: 'Hot-plug network interfaces to running VMs', state: 'GA' },
  ...
  Compute: {
    ...
    gates: [
      ...
      { name: 'VMLiveUpdateFeatures', description: 'Hot-plug CPU sockets to running VMs', state: 'GA' },
  ...
  Display: {
    ...
    gates: [
      ...
      { name: 'BochsDisplayForEFIGuests', description: 'Bochs display for EFI guests', state: 'GA' },
    ],
  },
  Other: {
    icon: 'mdi:cog',
    color: '#795548',
    gates: [
      { name: 'CommonInstancetypesDeploymentGate', description: 'Deploy common instance types', state: 'GA' },
      { name: 'MultiArchitecture', description: 'Multi-architecture VM scheduling', state: 'Alpha' },
    ],
  },
};
...
function SettingsPageMock() {
  const [enabledGates, setEnabledGates] = useState<string[]>([
    'Snapshot', 'VMExport', 'DataVolumes', 'LiveMigration', 'HotplugNICs',
    'NUMA', 'GPU', 'NonRoot', 'VMPersistentState', 'BochsDisplayForEFIGuests',
    'NetworkBindingPlugins', 'VMLiveUpdateFeatures', 'VolumeMigration',
  ...
  return (
    <StoryWrapper>
      <Typography variant="h4" gutterBottom>KubeVirt Configuration</Typography>
        ...
        <Box display="grid" gridTemplateColumns={{ xs: '1fr', md: 'repeat(2, 1fr)' }} gap={2}>
            ...
            <CardContent>
              <Box display="flex" alignItems="center" gap={2} mb={2}>
                ...
                <Tooltip title="Edit KubeVirt CR">
                  <IconButton size="small"><Icon icon="mdi:pencil" width={18} /></IconButton>
                </Tooltip>
              </Box>
          ...
          <Card variant="outlined">
            <CardContent>
              ...
              <Box display="flex" justifyContent="space-between" alignItems="center">
                <Typography variant="body2" color="text.secondary">Feature Gates</Typography>
      ...
      >
        ...
        >
          <Icon icon="mdi:puzzle" width={28} height={28} style={{ color: pluginFeaturesExpanded ? '#9c27b0' : '#9e9e9e' }} />
          <Typography variant="h6" flex={1}>Plugin Features</Typography>
          <Chip label="Headlamp Plugin" size="small" sx={{ backgroundColor: 'rgba(156, 39, 176, 0.2)', color: '#ce93d8', borderColor: '#9c27b0' }} variant="outlined" />
                ...
                <Box display="flex" justifyContent="space-between" alignItems="center">
                  <Box flex={1}>
                    <Box display="flex" alignItems="center" gap={1}>
                      <Typography variant="body1" fontWeight={500}>VM Delete Protection</Typography>
                      <IconButton size="small"><Icon icon="mdi:information-outline" width={20} /></IconButton>
                    </Box>
                  ...
                  <Box display="flex" alignItems="center" gap={2}>
                    <Chip label={deleteProtectionDeployed ? 'Deployed' : 'Not Deployed'} color={deleteProtectionDeployed ? 'success' : 'default'} size="small" variant="outlined" />
                    <Button
      ...
      >
        ...
        >
          <Icon icon="mdi:table-column-plus-after" width={28} height={28} style={{ color: labelColumnsExpanded ? '#4caf50' : '#9e9e9e' }} />
          <Typography variant="h6" flex={1}>VM List Custom Columns</Typography>
          <Chip label="Plugin Setting" size="small" sx={{ backgroundColor: 'rgba(76, 175, 80, 0.1)', color: '#4caf50', fontWeight: 500 }} />
        ...
        <Collapse in={labelColumnsExpanded}>
          <Box pt={2}>
            <Card variant="outlined" sx={{ mb: 2 }}>
              <CardContent>
                <Typography variant="subtitle2" fontWeight={500} mb={2}>Add New Column</Typography>
                <Grid container spacing={2} alignItems="flex-end">
                  <Grid item xs={12} sm={4}>
                    <TextField fullWidth size="small" label="Column Name" placeholder="e.g., App" />
                  </Grid>
                  <Grid item xs={12} sm={5}>
                    <TextField fullWidth size="small" label="Label Key" placeholder="e.g., app.kubernetes.io/name" />
                  </Grid>
                  <Grid item xs={12} sm={3}>
                    <Button fullWidth variant="contained" size="small" startIcon={<Icon icon="mdi:plus" />}>Add</Button>
                  </Grid>
                </Grid>
              </CardContent>
            </Card>
            <Card variant="outlined">
              <CardContent>
                <Typography variant="subtitle2" fontWeight={500} mb={2}>Current Custom Columns</Typography>
                <Box display="flex" flexDirection="column" gap={1}>
                  {labelColumns.map(col => (
                    <Box key={col.labelKey} display="flex" justifyContent="space-between" alignItems="center" p={1} sx={{ backgroundColor: 'rgba(0,0,0,0.02)', borderRadius: '4px' }}>
                      <Box>
                        <Typography variant="body2" fontWeight={500}>{col.label}</Typography>
                        <Typography variant="caption" color="text.secondary">{col.labelKey}</Typography>
                      </Box>
                      <IconButton size="small" color="error"><Icon icon="mdi:delete" width={20} /></IconButton>
                    </Box>
                  ))}
                </Box>
              </CardContent>
      ...
      >
        ...
        >
          <Icon icon="mdi:cog" width={28} height={28} style={{ color: generalConfigExpanded ? '#2196f3' : '#9e9e9e' }} />
          <Typography variant="h6" flex={1}>General Configuration</Typography>
          ...
          <Box p={2} pt={0}>
            <Card variant="outlined" sx={{ mb: 2 }}>
              <CardContent>
                <Box display="flex" justifyContent="space-between" alignItems="center">
                  <Box flex={1}>
                    <Typography variant="body1" fontWeight={500}>Common Instance Types Deployment</Typography>
                    <Typography variant="body2" color="text.secondary">Deploy predefined VM instance types (u1.small, u1.medium, etc.)</Typography>
                  </Box>
                  <FormControlLabel
                    control={<Switch checked={commonInstancetypes} onChange={(_, c) => setCommonInstancetypes(c)} sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: '#4caf50' }, '& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track': { backgroundColor: '#4caf50' } }} />}
                    label={<Typography variant="body2" sx={{ color: commonInstancetypes ? '#4caf50' : '#f44336', fontWeight: commonInstancetypes ? 600 : 400, minWidth: 85 }}>{commonInstancetypes ? 'Enabled' : 'Disabled'}</Typography>}
                  />
                </Box>
              </CardContent>
            </Card>
            <Card variant="outlined" sx={{ mb: 2 }}>
              <CardContent>
                <Typography variant="body1" fontWeight={500} mb={1}>Memory Overcommit</Typography>
                <Typography variant="body2" color="text.secondary" mb={2}>Percentage of memory to allocate beyond physical capacity</Typography>
                <Box display="flex" alignItems="center" gap={2}>
                  <Slider value={memoryOvercommit} onChange={(_, v) => setMemoryOvercommit(v as number)} min={100} max={200} step={10} valueLabelDisplay="auto" sx={{ flex: 1 }} />
                  <Chip label={`${memoryOvercommit}%`} size="small" color={memoryOvercommit > 100 ? 'warning' : 'default'} />
                  <Button variant="contained" size="small" sx={{ backgroundColor: '#4caf50', '&:hover': { backgroundColor: '#45a049' } }}>Apply</Button>
                </Box>
              </CardContent>
            </Card>
            <Card variant="outlined">
              <CardContent>
                <Typography variant="body1" fontWeight={500} mb={1}>Eviction Strategy</Typography>
                <Box display="flex" alignItems="center" gap={2}>
                  <FormControl size="small" sx={{ flex: 1 }}>
                    <InputLabel>Strategy</InputLabel>
                    <Select value={evictionStrategy} label="Strategy" onChange={e => setEvictionStrategy(e.target.value)}>
      ...
      <Box sx={{ border: 1, borderColor: 'divider', borderRadius: '4px', p: 2 }}>
        <Typography variant="h6" gutterBottom>Feature Gates</Typography>
          ...
          {Object.entries(FEATURE_GATE_CATEGORIES).map(([category, { icon, color, gates }]) => (
            ...
            >
              <CardContent sx={{ pb: '12px !important' }}>
                <Box
                  display="flex" alignItems="center" gap={1} mb={activeCategory === category ? 2 : 0}
                  onClick={() => setActiveCategory(activeCategory === category ? null : category)}
                >
                  <Icon icon={icon} width={22} color={color} />
                  <Typography variant="subtitle1" fontWeight={600} flex={1}>{category}</Typography>
                  <Chip
                    label={`${gates.filter(g => enabledGates.includes(g.name)).length}/${gates.length}`}
                    size="small"
                    sx={{ bgcolor: `${color}20`, color, fontWeight: 600 }}
                  />
                  <Icon icon={activeCategory === category ? 'mdi:chevron-up' : 'mdi:chevron-down'} width={20} />
                </Box>
                <Collapse in={activeCategory === category}>
                  {gates.map(gate => (
                    ...
                    >
                      <Box flex={1}>
                        <Box display="flex" alignItems="center" gap={1}>
                          <Typography variant="body2" fontWeight={500}>{gate.name}</Typography>
                          <Chip label={gate.state} size="small" color={STATE_COLORS[gate.state] || 'default'} variant="outlined" sx={{ height: 20, fontSize: '0.65rem' }} />
                        </Box>
                        <Typography variant="caption" color="text.secondary">{gate.description}</Typography>
                      </Box>
                      <Switch
                        size="small"
