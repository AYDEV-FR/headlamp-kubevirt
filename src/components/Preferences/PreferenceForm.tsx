...
export default function PreferenceForm({ resource, onChange, editMode = false }: PreferenceFormProps) {
  ...
  const isMinCPUEmpty = hasMinRequirements && (!resource.spec?.requirements?.cpu?.guest || resource.spec?.requirements?.cpu?.guest === '');
  const isMinMemoryEmpty = hasMinRequirements && (!resource.spec?.requirements?.memory?.guest || resource.spec?.requirements?.memory?.guest === '' || minMemory === '');

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      {/* Basic Information */}
      <FormSection icon="mdi:information-outline" title="Basic Information" color="other" spacing={2}>
          <Grid item xs={12}>
            <TextField
              fullWidth
              required
              ...
              helperText={editMode ? 'Name cannot be changed' : 'Unique identifier for this preference (e.g., my.custom.os)'}
              disabled={editMode}
            />
          </Grid>
      ...
      <FormSection icon="mdi:alert-circle-outline" title="Minimum Requirements" color="compute" spacing={2}>
          <Grid item xs={12}>
            <FormControlLabel
              control={
                <Switch
