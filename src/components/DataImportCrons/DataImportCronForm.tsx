...
export default function DataImportCronForm({ resource, onChange, editMode = false }: DataImportCronFormProps) {
  ...
  // Get source type
  const sourceType = resource.spec?.source
    ? Object.keys(resource.spec.source)[0]
    : 'registry';
      ...
      <FormSection icon="mdi:information-outline" title="Basic Information" color="other">
          ...
          <Grid item xs={12} md={6}>
            <TextField
              ...
              helperText={editMode ? 'Namespace cannot be changed' : 'Namespace for the DataImportCron'}
              disabled={editMode}
            >
              {namespaces.map((ns) => (
      ...
      <FormSection icon="mdi:harddisk" title="Storage Configuration" color="storage">
          ...
          <Grid item xs={12} md={6}>
            <TextField
              ...
              label="Access Mode"
              value={
                resource.spec?.template?.spec?.pvc?.accessModes?.[0] || 'ReadWriteOnce'
              }
